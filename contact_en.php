...
if(isset($_POST)) {
    ...
    if(mail($recipient, 'Zpráva z formuláře - '.$email_title, $email_body, $headers)) {
        ...
        $msg = "<div class='alert rounded-0 alert-dismissible alert-success slide'>Thanks for your message. We will answer as soon as possible!  <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
        </div>";
        
    } else {
        
        $msg = "<div class='alert rounded-0 alert-dismissible alert-danger slide'>Something went wrong. Try again please.<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
        </div>";
    }
